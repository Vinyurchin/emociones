...
def analyze_face(image_path):
    ...
    try:
        ...
        annotated_points = {
            ...
            "extremos_ojo_izquierdo": [33, 226],  # Extremo izquierdo del ojo y ceja
            "extremos_ojo_derecho": [263, 446],  # Extremo derecho del ojo y ceja
            "centro_ojo": [168]  # Centro de los ojos
        }

        for category, indices in annotated_points.items():
            for idx in indices:
                # Escalar coordenadas de los puntos faciales
                x = int(face_landmarks.landmark[idx].x * width)
                y = int(face_landmarks.landmark[idx].y * height)
                # Dibujar cada punto como un pequeño círculo rojo
